...
/* ─── Particles Effect ───────────────────────────────────────────── */
function Particles(){return <div style={{position:"absolute",inset:0,overflow:"hidden"}}>{Array.from({length:12}).map((_,i)=><div key={i} style={{position:"absolute",width:2+Math.random()*3,height:2+Math.random()*3,background:`var(--accent)${Math.round(10+Math.random()*25).toString(16)}`,borderRadius:"50%",left:`${Math.random()*100}%`,top:`${Math.random()*100}%`,animation:`float ${4+Math.random()*6}s ease-in-out infinite ${Math.random()*5}s`}} />)}</div>;}
...
function AboutTrustBadge({onClick}){const[h,setH]=useState(false);return(<button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{display:"inline-flex",alignItems:"center",gap:8,background:h?"rgba(119,178,88,0.08)":"rgba(119,178,88,0.04)",border:`1px solid rgba(119,178,88,${h?0.25:0.12})`,borderRadius:8,padding:"10px 16px",cursor:"pointer",transition:"all 0.25s ease",transform:h?"translateY(-1px)":"none",width:"100%"}}><svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L3 7v5c0 5.25 3.75 10.15 9 11.25C17.25 22.15 21 17.25 21 12V7l-9-5z"/><path d="M9 12l2 2 4-4"/></svg><span style={{fontFamily:"var(--font-mono)",fontSize:12,color:"var(--c-text2)",textAlign:"left",flex:1}}>{ABOUT_BIO.oneLiner}</span><span style={{fontSize:12,color:"var(--accent)",opacity:h?1:0.5}}>→</span></button>);}

function AboutPanel({open,onClose}){const[v,setV]=useState(false);const[s,setS]=useState(false);useEffect(()=>{if(open){setV(true);setTimeout(()=>setS(true),30);}else{setS(false);setTimeout(()=>setV(false),350);}},[open]);if(!v)return null;return(<div onClick={onClose} style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:s?"rgba(0,0,0,0.7)":"rgba(0,0,0,0)",backdropFilter:s?"blur(8px)":"blur(0)",transition:"all 0.35s ease",padding:20}}><div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:560,maxHeight:"85vh",overflowY:"auto",background:"var(--c-card)",border:"1px solid rgba(119,178,88,0.1)",borderRadius:16,opacity:s?1:0,transform:s?"translateY(0) scale(1)":"translateY(16px) scale(0.97)",transition:"all 0.35s cubic-bezier(0.16,1,0.3,1)",boxShadow:"0 24px 80px rgba(0,0,0,0.5)"}}><div style={{padding:"28px 28px 20px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",borderBottom:"1px solid var(--c-border)"}}><div><div style={{fontSize:11,fontWeight:600,letterSpacing:2.5,textTransform:"uppercase",color:"var(--accent)"}}>About Prudence</div><div style={{fontSize:12,color:"var(--c-sub)",marginTop:2}}>Safety & Environmental Consulting</div></div><button onClick={onClose} style={{background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:8,width:34,height:34,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--c-sub)"}}>✕</button></div><div style={{padding:"24px 28px 20px"}}>{ABOUT_BIO.paragraphs.map((p,i)=>(<p key={i} style={{fontSize:14,lineHeight:1.75,color:i===0?"var(--c-text)":"var(--c-text2)",margin:0,marginBottom:i<2?18:0}}>{i===0?<><strong style={{color:"var(--c-text)",fontWeight:600}}>Tsidi Tamakloe, CSP</strong>{p.substring(p.indexOf(" is the founder"))}</>:p}</p>))}</div><div style={{padding:"16px 28px 24px",borderTop:"1px solid var(--c-border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}><span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"var(--c-dim)"}}>CSP · BCSP #38426</span><a href={ABOUT_BIO.website} target="_blank" rel="noopener noreferrer" style={{fontSize:13,color:"var(--accent)",textDecoration:"none",padding:"6px 14px",borderRadius:6,border:"1px solid rgba(119,178,88,0.2)",background:"rgba(119,178,88,0.06)"}}>prudencesafety.com ↗</a></div></div></div>);}
...
export default function MobileApp() {
  ...
  const TOUR = [
    {icon:"drop",color:"var(--accent)",title:"Welcome to HydroScan",sub:"Drinking Water Quality Intelligence",body:"HydroScan guides you through complete drinking water assessments — from field walkthrough to lab results evaluation. Every finding is evaluated against EPA, WHO, and ASHRAE standards with full regulatory traceability."},
    {icon:"search",color:"var(--accent)",title:"Field Assessment",sub:"Mode 1 — On-site walkthrough",body:"Guided questions cover water source type (public/private well), well construction, plumbing materials, fixture age, water heater settings, stagnation risks, and visual observations. Conditional branching adapts to your specific scenario."},
    {icon:"flask",color:"var(--accent2)",title:"Lab Results Evaluation",sub:"Mode 2 — Compliance analysis",body:"Enter lab results manually or use quick-add panels for common test packages. The compliance engine evaluates 50+ parameters against EPA MCLs, Action Levels, WHO Guidelines, and state-specific standards — instantly."},
    {icon:"shield",color:"var(--success)",title:"Tiered Compliance",sub:"Not a score — a classification",body:"Results are classified as Immediate Action (MCL violations), Advisory (approaching limits), Monitor (secondary exceedances), or Compliant. A single MCL violation flags the entire assessment. No averaging. No hiding violations."},
    {icon:"drop",color:"var(--danger)",title:"PFAS Analysis",sub:"6 compounds + Hazard Index",body:"Full EPA PFAS NPDWR evaluation: PFOA and PFOS at 4 ppt, PFHxS/PFNA/GenX at 10 ppt, plus the Hazard Index calculation for co-occurring PFAS mixtures. State-specific limits for NJ, CA, MA, NH, MI, and VT."},
    {icon:"chain",color:"var(--sev-high)",title:"Root Cause Analysis",sub:"Causal chain engine",body:"The engine links lab results to field observations: lead service line + low pH + stagnation = lead contamination chain. Microbial, Legionella, nitrate, and corrosion pathways are identified with confidence ratings."},
    {icon:"clip",color:"var(--accent)",title:"Sampling Plans & COC",sub:"Hypothesis-driven + chain of custody",body:"Sampling recommendations are generated from walkthrough findings with EPA method numbers, hold times, and preservation requirements. The free Chain of Custody form auto-populates from your sampling plan."},
    {icon:"chart",color:"var(--warn)",title:"Historical Trending",sub:"Track changes over time",body:"Every evaluation is saved automatically. After two or more tests, the Trending tab shows sparkline charts for each parameter — is your lead going up or down? MCL limit lines show proximity to violations."},
    {icon:"pipe",color:"var(--accent)",title:"Collection Guides",sub:"Step-by-step sampling protocols",body:"Visual guides for Lead First-Draw (EPA 3Ts protocol), Bacteriological (sterile technique, 6-hour hold), and PFAS (contamination prevention — no waterproof clothing, HDPE only). Printable from the field."},
    {icon:"bolt",color:"var(--accent)",title:"Ready to Start",sub:"Choose your mode",body:"Tap Field Assessment for on-site walkthroughs or Lab Results for direct compliance evaluation. The Chain of Custody form is always available from the dashboard — free for everyone."},
  ];
  ...
  // Apply the theme to <html data-theme> whenever it changes (the inline
  // boot script in index.html sets it pre-paint; this keeps it in sync).
  ...
  useEffect(() => {
    (async () => {
      ...
      const savedTheme = await STO.get("hydroscan-theme");
  ...
  const toggleTheme = async () => { const next = theme === "dark" ? "light" : "dark"; setTheme(next); await STO.set("hydroscan-theme", next); haptic("light"); };
  // Tint helper — alpha over a CSS color, var()-safe (can't append hex alpha
  // to a var(), so use color-mix). pct is the color's opacity percentage.
    ...
    <div style={{paddingTop:12,paddingBottom:100}}>
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <span style={{fontSize:12,color:"var(--c-sub)",fontFamily:"var(--font-mono)"}}>{qIdx+1} of {visQs.length}</span>
        ...
        <div style={{height:4,background:"var(--c-surface)",borderRadius:2,overflow:"hidden"}}>
          <div style={{height:"100%",width:`${progress}%`,background:"linear-gradient(90deg,var(--cta1),var(--cta2))",borderRadius:2,transition:"width .4s ease",boxShadow:"0 0 8px color-mix(in srgb, var(--accent) 25%, transparent)"}} />
        </div>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:24,flexWrap:"wrap"}}>
        {secs.map((s,i)=><span key={s} style={{padding:"7px 14px",borderRadius:20,fontSize:12,fontWeight:600,fontFamily:"var(--font-mono)",background:i===secIdx?"color-mix(in srgb, var(--accent) 8%, transparent)":"transparent",color:i===secIdx?"var(--accent)":i<secIdx?"var(--c-sub)":"var(--c-faint)",border:`1px solid ${i===secIdx?"color-mix(in srgb, var(--accent) 19%, transparent)":"transparent"}`}}>{s}</span>)}
      </div>
      <div key={q.id} style={{animation:"fadeUp .4s cubic-bezier(.22,1,.36,1)"}}>
        <div style={{width:52,height:52,borderRadius:14,background:"color-mix(in srgb, var(--accent) 6%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 13%, transparent)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,marginBottom:18}}>{q.ic}</div>
        <h2 style={{fontSize:26,fontWeight:700,lineHeight:1.25,margin:0,marginBottom:8,letterSpacing:"-0.5px"}}>{q.q}</h2>
        {q.ref&&<div style={{display:"inline-flex",gap:7,padding:"7px 14px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:8,marginBottom:20,marginTop:6}}><span style={{fontSize:13,color:"var(--accent)"}}>📐</span><span style={{fontSize:12,color:"var(--c-sub)",fontFamily:"var(--font-mono)",lineHeight:1.4}}>{q.ref}</span></div>}
        {!q.ref&&<div style={{height:16}} />}

        {q.t==="text"&&<input type="text" value={data[q.id]||""} onChange={e=>setField(q.id,e.target.value)} placeholder={q.ph||"Type..."} autoFocus onKeyDown={e=>{if(e.key==="Enter"&&data[q.id])goNext();}} style={{width:"100%",padding:"16px 18px",background:"var(--c-card)",border:"1.5px solid var(--c-border)",borderRadius:12,color:"var(--c-text)",fontSize:17,fontFamily:"var(--font-sans)",fontWeight:500,outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--c-border)"} />}
        {q.t==="num"&&<div style={{position:"relative"}}><input type="number" value={data[q.id]||""} onChange={e=>setField(q.id,e.target.value)} placeholder={q.ph||"Enter..."} autoFocus onKeyDown={e=>{if(e.key==="Enter"&&data[q.id])goNext();}} style={{width:"100%",padding:"16px 18px",paddingRight:q.u?70:18,background:"var(--c-card)",border:"1.5px solid var(--c-border)",borderRadius:12,color:"var(--c-text)",fontSize:17,fontFamily:"var(--font-sans)",fontWeight:500,outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--c-border)"} />{q.u&&<span style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",color:"var(--c-dim)",fontSize:14,fontFamily:"var(--font-mono)"}}>{q.u}</span>}</div>}
        {q.t==="ch"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>{q.opts.map((o,i)=>{const sel=data[q.id]===o;return(<button key={o} onClick={()=>{haptic("light");setField(q.id,o);setTimeout(goNext,250);}} style={{padding:"15px 18px",textAlign:"left",background:sel?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--c-card)",border:`1.5px solid ${sel?"var(--accent)":"var(--c-border)"}`,borderRadius:12,color:sel?"var(--accent)":"var(--c-text)",fontSize:16,fontFamily:"var(--font-sans)",fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:12,animation:`fadeUp .3s ${i*.04}s cubic-bezier(.22,1,.36,1) both`}}><div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${sel?"var(--accent)":"var(--c-border2)"}`,background:sel?"var(--accent)":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>{sel&&<svg width="10" height="10" viewBox="0 0 10 10"><path d="M2 5L4.5 7.5L8 3" stroke="var(--on-accent)" strokeWidth="1.5" fill="none"/></svg>}</div>{o}</button>);})}</div>}
        {q.t==="multi"&&<div style={{display:"flex",flexWrap:"wrap",gap:8}}>{q.opts.map((o,i)=>{const sel=(data[q.id]||[]).includes(o);return(<button key={o} onClick={()=>{haptic("light");setField(q.id,sel?(data[q.id]||[]).filter(x=>x!==o):[...(data[q.id]||[]),o]);}} style={{padding:"12px 16px",background:sel?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--c-card)",border:`1.5px solid ${sel?"var(--accent)":"var(--c-border)"}`,borderRadius:10,color:sel?"var(--accent)":"var(--c-text2)",fontSize:14,cursor:"pointer",fontFamily:"inherit",animation:`fadeUp .25s ${i*.03}s cubic-bezier(.22,1,.36,1) both`}}>{sel?"✓ ":""}{o}</button>);})}</div>}
        {q.t==="ta"&&<textarea value={data[q.id]||""} onChange={e=>setField(q.id,e.target.value)} placeholder={q.ph||"Describe..."} rows={3} style={{width:"100%",padding:"14px 16px",background:"var(--c-card)",border:"1.5px solid var(--c-border)",borderRadius:12,color:"var(--c-text)",fontSize:15,fontFamily:"inherit",outline:"none",resize:"vertical",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--c-border)"} />}
        {q.photo&&<div style={{marginTop:12,fontSize:12,color:"var(--c-dim)"}}>📷 Photo capture available on deployed version</div>}
      </div>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:30,gap:10}}>
        <button onClick={goPrev} style={{padding:"12px 20px",background:"transparent",border:`1.5px solid ${qIdx>0?"var(--c-border2)":"transparent"}`,borderRadius:10,color:qIdx>0?"var(--c-sub)":"transparent",fontSize:14,cursor:qIdx>0?"pointer":"default",fontFamily:"inherit"}}>← Back</button>
        <div style={{display:"flex",gap:8}}>
          {q.sk&&<button onClick={goNext} style={{padding:"12px 20px",background:"transparent",border:"1.5px solid var(--c-border2)",borderRadius:10,color:"var(--c-sub)",fontSize:14,fontWeight:500,cursor:"pointer",fontFamily:"inherit"}}>Skip</button>}
          {qIdx===visQs.length-1
            ? <button onClick={onFinish} style={{padding:"13px 26px",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:10,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px color-mix(in srgb, var(--success) 19%, transparent)"}}>{finishLabel}</button>
            : q.t!=="ch" ? <button onClick={goNext} style={{padding:"13px 26px",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:10,color:"#fff",fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:(!q.req||data[q.id])?1:.3,boxShadow:"0 4px 16px color-mix(in srgb, var(--accent) 19%, transparent)"}}>Continue →</button> : null}
        </div>
      </div>
    </div>
  );};
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text)",fontFamily:"var(--font-sans)"}}>      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0}}><div style={{position:"absolute",top:"-20%",left:"-10%",width:"50%",height:"50%",background:"radial-gradient(circle,color-mix(in srgb, var(--accent) 2%, transparent) 0%,transparent 70%)",filter:"blur(60px)"}} /><div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(circle at 1px 1px, color-mix(in srgb, var(--accent) 2%, transparent) 1px, transparent 0)",backgroundSize:"32px 32px"}} /></div>

      {/* Header */}
      <header style={{position:"sticky",top:0,zIndex:100,height:"calc(50px + env(safe-area-inset-top,0px))",boxSizing:"border-box",background:"var(--header-bg)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid var(--c-border)",padding:"env(safe-area-inset-top,0px) 16px 0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontSize:15,fontWeight:600,lineHeight:1.1}}>Hydro<span style={{color:"var(--accent)",fontWeight:800}}>Scan</span></div><div style={{fontSize:11,color:"var(--c-sub)",fontFamily:"var(--font-mono)"}}>by Prudence EHS</div></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
      ...
      {/* Milestone */}
      {milestone&&<div style={{position:"fixed",inset:0,background:"var(--scrim-full)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",animation:"fadeIn .3s ease"}}><div style={{textAlign:"center",animation:"milestoneIn .5s cubic-bezier(.22,1,.36,1)"}}><div style={{marginBottom:16,display:"flex",justifyContent:"center"}}><div style={{width:72,height:72,borderRadius:20,background:"color-mix(in srgb, var(--accent) 7%, transparent)",border:"1.5px solid color-mix(in srgb, var(--accent) 19%, transparent)",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={milestone.icon} s={36} c="var(--accent)" w={2} /></div></div><div style={{fontSize:24,fontWeight:800,color:"var(--c-text)",fontFamily:"var(--font-sans)",letterSpacing:"-0.5px"}}>{milestone.title}</div><div style={{fontSize:14,color:"var(--accent)",fontFamily:"var(--font-mono)",marginTop:8}}>{milestone.sub}</div><div style={{width:48,height:3,background:"linear-gradient(90deg,var(--cta2),var(--cta1))",borderRadius:2,margin:"16px auto 0",animation:"milestoneBar 1.2s ease"}} /></div></div>}
      ...
      {showGuide&&COLLECTION_GUIDES[showGuide]&&(
        <div style={{position:"fixed",inset:0,background:"#000000DD",zIndex:250,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowGuide(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:640,maxHeight:"88vh",overflow:"auto",padding:"24px 20px 40px",animation:"slideUp .3s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:18,fontWeight:700,fontFamily:"var(--font-sans)"}}>{COLLECTION_GUIDES[showGuide].title}</div>
              <button onClick={()=>setShowGuide(null)} style={{width:32,height:32,borderRadius:8,border:"1px solid var(--c-border)",background:"transparent",color:"var(--c-sub)",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
            </div>
            <div style={{fontSize:12,color:"var(--accent)",fontFamily:"var(--font-mono)",marginBottom:20}}>{COLLECTION_GUIDES[showGuide].std}</div>

            {COLLECTION_GUIDES[showGuide].steps.map((step,i) => (
              <div key={i} style={{display:"flex",gap:14,marginBottom:14}}>
                <div style={{width:36,height:36,borderRadius:10,background:"color-mix(in srgb, var(--accent) 6%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 13%, transparent)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,fontWeight:800,color:"var(--accent)",fontFamily:"var(--font-mono)"}}>{step.n}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:14,color:"var(--c-text)",lineHeight:1.6}}>{step.text}</div>
                </div>
              </div>
            ))}

            {COLLECTION_GUIDES[showGuide].notes&&(
              <div style={{marginTop:16,padding:"14px 16px",background:"color-mix(in srgb, var(--warn) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--warn) 13%, transparent)",borderRadius:12}}>
                <div style={{fontSize:12,fontWeight:700,color:"var(--warn)",marginBottom:4}}>⚠ Important Notes</div>
                <div style={{fontSize:13,color:"var(--c-text2)",lineHeight:1.6}}>{COLLECTION_GUIDES[showGuide].notes}</div>
              </div>
            )}
          </div>
        </div>
          ...
          <div onClick={e=>e.stopPropagation()} style={{position:"absolute",top:0,left:0,bottom:0,width:"min(86vw,312px)",background:"var(--bg)",borderRight:"1px solid var(--border)",boxShadow:"24px 0 70px #00000090",animation:"drawerIn .26s cubic-bezier(.16,1,.3,1)",display:"flex",flexDirection:"column",overflowY:"auto",paddingTop:"calc(env(safe-area-inset-top,0px) + 16px)",paddingBottom:"calc(env(safe-area-inset-bottom,0px) + 24px)"}}>
            {(()=>{
              ...
              return (<>
                ...
                {div}
                {Row({label:"Analyze Lab Results",icon:"flask",tone:"var(--accent2)",onClick:()=>{setActiveProjectId(null);startLab();}})}
                {Row({label:"Generate Chain of Custody",icon:"clip",tone:"var(--success)",onClick:()=>{setActiveProjectId(null);initCOC();}})}
                {div}
      ...
      {showTos&&<div style={{position:"fixed",inset:0,background:"#000000DD",zIndex:400,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>tosAccepted&&setShowTos(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:640,maxHeight:"90vh",overflow:"auto",padding:"28px 24px 40px",animation:"slideUp .3s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
            <div style={{fontSize:20,fontWeight:800,fontFamily:"var(--font-sans)"}}>Terms of Service</div>
            {tosAccepted&&<button onClick={()=>setShowTos(false)} style={{width:32,height:32,borderRadius:8,border:"1px solid var(--c-border)",background:"transparent",color:"var(--c-sub)",fontSize:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>}
          </div>
          <div style={{fontSize:12,color:"var(--c-sub)",fontFamily:"var(--font-mono)",marginBottom:20}}>Last updated: April 2026 · HydroScan by Prudence Safety & Environmental Consulting, LLC</div>
          {[
          ...
          ].map((s,i)=><div key={i} style={{marginBottom:14}}><div style={{fontSize:14,fontWeight:700,color:"var(--c-text)",marginBottom:3}}>{s.t}</div><div style={{fontSize:13,color:"var(--c-sub)",lineHeight:1.7}}>{s.b}</div></div>)}
          {!tosAccepted?<button onClick={acceptTos} style={{width:"100%",padding:"16px 0",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:12,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginTop:12}}>I Accept These Terms</button>:<div style={{textAlign:"center",fontSize:13,color:"var(--success)",fontFamily:"var(--font-mono)"}}>✓ Accepted</div>}
        </div>
      </div>}
      ...
      {panel&&<div style={{position:"fixed",inset:0,background:"#000000DD",zIndex:250,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setPanel(null)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:640,maxHeight:"88vh",overflow:"auto",padding:"24px 20px 40px",animation:"slideUp .3s ease"}}>
          ...
          {panel==="about"&&<div>
            <div style={{textAlign:"center",marginBottom:24}}><Logo s={56} /><div style={{fontSize:28,fontWeight:800,fontFamily:"var(--font-sans)",marginTop:8}}>Hydro<span style={{color:"var(--accent)"}}>Scan</span></div><div style={{fontSize:12,color:"var(--c-sub)",fontFamily:"var(--font-mono)",marginTop:4}}>by Prudence EHS · v{VER}</div></div>
            <div style={{fontSize:14,color:"var(--c-sub)",lineHeight:1.7,marginBottom:16}}>HydroScan is a standards-driven drinking water assessment platform. It evaluates lab results against EPA, WHO, and state-specific standards, generates hypothesis-driven sampling plans, and provides defensible documentation for private wells and building water systems.</div>
            {[{l:"Developed by",v:"Prudence Safety & Environmental Consulting, LLC"},{l:"Location",v:"Germantown, MD"},{l:"Website",v:"prudencesafety.com"},{l:"Standards",v:"EPA SDWA MCLs · PFAS NPDWR 2024 · Lead & Copper Rule · WHO GDWQ · ASHRAE 188 · EPA 3Ts"},{l:"Parameters",v:"50+ including 6 PFAS with Hazard Index calculation"}].map((r,i)=><div key={i} style={{display:"flex",gap:12,marginBottom:8,fontSize:13}}><span style={{color:"var(--c-dim)",fontFamily:"var(--font-mono)",fontSize:11,minWidth:85,flexShrink:0}}>{r.l}</span><span style={{color:"var(--c-text2)",lineHeight:1.5}}>{r.v}</span></div>)}
          </div>}

          {panel==="settings"&&<div>
            <div style={{fontSize:13,color:"var(--sub)",marginBottom:16}}>Manage your profile, references, and app information.</div>
            {/* Appearance — light is the default; toggle to dark */}
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"13px 14px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:R.lg,marginBottom:8}}>
              <span style={{width:36,height:36,borderRadius:R.md,background:"color-mix(in srgb, var(--accent) 12%, transparent)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n={theme==="dark"?"moon":"sun"} s={18} c="var(--accent)"/></span>
              <span style={{flex:1,minWidth:0}}><span style={{display:"block",fontSize:14,fontWeight:700,color:"var(--text)"}}>Dark mode</span><span style={{display:"block",fontSize:12,color:"var(--sub)",marginTop:1}}>{theme==="dark"?"On — dark theme":"Off — light theme"}</span></span>
              <button role="switch" aria-checked={theme==="dark"} aria-label="Toggle dark mode" onClick={toggleTheme} className="tap" style={{flexShrink:0,width:46,height:28,borderRadius:999,border:"none",cursor:"pointer",padding:0,position:"relative",background:theme==="dark"?"var(--accent-fill)":"var(--c-border2)",transition:"background .2s ease"}}>
                <span style={{position:"absolute",top:3,left:theme==="dark"?21:3,width:22,height:22,borderRadius:999,background:"#fff",boxShadow:"0 1px 3px rgba(0,0,0,0.3)",transition:"left .2s ease"}}/>
              </button>
            </div>
      ...
      {/* Guided Tour */}
      {showTour&&(
        <div style={{position:"fixed",inset:0,background:"var(--scrim-full)",zIndex:350,display:"flex",flexDirection:"column",animation:"fadeIn .3s ease"}}>
          <div style={{display:"flex",justifyContent:"center",gap:6,padding:"20px 0 10px"}}>
            {TOUR.map((_,i)=><div key={i} onClick={()=>setTourStep(i)} style={{width:tourStep===i?24:8,height:8,borderRadius:4,background:tourStep===i?"var(--accent)":"var(--c-border)",transition:"all .3s",cursor:"pointer"}} />)}
          </div>
          <div style={{textAlign:"right",padding:"0 24px"}}>
            <button onClick={()=>setShowTour(false)} style={{background:"none",border:"none",color:"var(--c-dim)",fontSize:13,cursor:"pointer",fontFamily:"inherit",padding:"8px 0"}}>Skip →</button>
          </div>
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 32px"}}>
            <div key={tourStep} style={{textAlign:"center",maxWidth:400,animation:"fadeUp .4s cubic-bezier(.22,1,.36,1)"}}>
              <div style={{display:"flex",justifyContent:"center",marginBottom:24}}>
                <div style={{width:80,height:80,borderRadius:22,background:mix(TOUR[tourStep].color,7),border:`1.5px solid ${mix(TOUR[tourStep].color,19)}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              <div style={{fontSize:14,color:"var(--accent)",fontFamily:"var(--font-mono)",marginBottom:16}}>{TOUR[tourStep].sub}</div>
              <div style={{fontSize:15,color:"var(--c-sub)",lineHeight:1.75}}>{TOUR[tourStep].body}</div>
            </div>
          </div>
          <div style={{padding:"20px 24px 36px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <button onClick={()=>setTourStep(Math.max(0,tourStep-1))} style={{padding:"12px 20px",background:"none",border:`1px solid ${tourStep===0?"transparent":"var(--c-border)"}`,borderRadius:10,color:tourStep===0?"transparent":"var(--c-sub)",fontSize:14,cursor:tourStep===0?"default":"pointer",fontFamily:"inherit"}}>← Back</button>
            {tourStep < TOUR.length - 1 ? (
              <button onClick={()=>{setTourStep(tourStep+1);haptic("light");}} style={{padding:"12px 28px",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:10,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px color-mix(in srgb, var(--accent) 19%, transparent)"}}>Next</button>
            ) : (
              <button onClick={()=>{setShowTour(false);haptic("success");}} style={{padding:"12px 28px",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:10,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px color-mix(in srgb, var(--success) 19%, transparent)"}}>Get Started</button>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{maxWidth:620,margin:"0 auto",padding:"0 16px 88px",position:"relative",zIndex:1}}>
          ...
          <div style={{paddingTop:18,paddingBottom:96}}>
            ...
            <div className="hero-card" onTouchStart={()=>{}} style={{position:"relative",borderRadius:22,background:"var(--card)",border:"1.5px solid var(--accent-fill)",padding:"24px 22px",marginBottom:28,boxShadow:"0 0 16px -3px color-mix(in srgb, var(--accent-fill) 42%, transparent)",animation:"fadeUp .4s ease"}}>
              ...
              <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                <button onClick={()=>{setActiveProjectId(null);startField();}} className="tap" style={{padding:"10px 18px",borderRadius:999,border:"none",background:"var(--accent-fill)",color:"var(--on-accent-fill)",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 16px rgba(119,178,88,0.4)"}}>Start sampling</button>
                <button onClick={()=>{setActiveProjectId(null);startSmart();}} className="tap" style={{padding:"10px 18px",borderRadius:999,border:"1px solid color-mix(in srgb, var(--accent) 45%, transparent)",background:"transparent",color:"var(--accent)",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Quick test</button>
              </div>
            </div>
        ...
        {view==="reference"&&(
          <div style={{paddingTop:24,paddingBottom:96}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:24}}>
              {[
                {i:"shield",l:"EPA SDWA MCLs",c:"var(--accent)"},{i:"drop",l:"PFAS (6 compounds + HI)",c:"var(--accent)"},
                {i:"pipe",l:"Lead & Copper Rule",c:"var(--sev-high)"},{i:"bacteria",l:"Microbial Analysis",c:"var(--danger)"},
                {i:"bldg",l:"ASHRAE 188 Legionella",c:"var(--accent2)"},{i:"well",l:"Private Well Assessment",c:"var(--accent)"},
                {i:"chart",l:"Historical Trending",c:"var(--warn)"},{i:"clip",l:"Collection Guides",c:"var(--sub)"},
              ].map((cap,idx)=>(
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {["Public water (city/county)","Private well","Not sure"].map(o=>(
                  <button key={o} onClick={()=>setSmart(p=>({...p,source:o==="Private well"?"Private well":"Public water"}))} style={{padding:"14px 16px",textAlign:"left",background:smart.source===(o==="Private well"?"Private well":"Public water")&&(o!=="Not sure"||!smart.source)?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--card)",border:`1.5px solid ${smart.source===(o==="Private well"?"Private well":"Public water")&&o!=="Not sure"?"var(--accent)":"var(--border)"}`,borderRadius:12,color:smart.source===(o==="Private well"?"Private well":"Public water")&&o!=="Not sure"?"var(--accent)":"var(--text)",fontSize:15,cursor:"pointer",fontFamily:"inherit",fontWeight:500}}>{o}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {["Home","Pre-1986 home","School / Daycare","Apartment complex","Office / Commercial","Healthcare","Other"].map(o=>{const sel=smart.building===o;return(
                  <button key={o} onClick={()=>setSmart(p=>({...p,building:o}))} style={{padding:"10px 16px",background:sel?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--card)",border:`1.5px solid ${sel?"var(--accent)":"var(--border)"}`,borderRadius:10,color:sel?"var(--accent)":"var(--text)",fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>{o}</button>
                );})}
              </div>
            </div>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {["Routine / annual","Buying or selling property","Someone got sick","Water looks/tastes/smells wrong","Contamination nearby","Required by regulation","Just want to know"].map(o=>{const sel=smart.trigger===o;return(
                  <button key={o} onClick={()=>setSmart(p=>({...p,trigger:o}))} style={{padding:"10px 16px",background:sel?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--card)",border:`1.5px solid ${sel?"var(--accent)":"var(--border)"}`,borderRadius:10,color:sel?"var(--accent)":"var(--text)",fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>{o}</button>
                );})}
              </div>
            </div>
            ...
            <div style={{marginBottom:24}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8}}>
                {["Lead worry","PFAS concern","Bad taste/smell","Discoloration","Staining","Illness","Skin irritation","None — just testing"].map(o=>{const sel=(smart.concerns||[]).includes(o);return(
                  <button key={o} onClick={()=>setSmart(p=>({...p,concerns:sel?p.concerns.filter(x=>x!==o):o==="None — just testing"?["None — just testing"]:[...(p.concerns||[]).filter(x=>x!=="None — just testing"),o]}))} style={{padding:"10px 16px",background:sel?"color-mix(in srgb, var(--accent) 7%, transparent)":"var(--card)",border:`1.5px solid ${sel?"var(--accent)":"var(--border)"}`,borderRadius:10,color:sel?"var(--accent)":"var(--text)",fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>{sel?"✓ ":""}{o}</button>
                );})}
              </div>
            </div>

            <button onClick={generateSmartPlan} disabled={!smart.source||!smart.trigger} style={{width:"100%",padding:"16px 0",background:smart.source&&smart.trigger?"linear-gradient(135deg,var(--cta1),var(--cta2))":"var(--border)",border:"none",borderRadius:12,color:smart.source&&smart.trigger?"#fff":"var(--dim)",fontSize:16,fontWeight:700,cursor:smart.source&&smart.trigger?"pointer":"default",fontFamily:"inherit",boxShadow:smart.source&&smart.trigger?"0 4px 20px color-mix(in srgb, var(--accent) 19%, transparent)":"none"}}>Generate Sampling Plan →</button>
          </div>
        )}
          ...
          <div style={{paddingTop:28,paddingBottom:100,animation:"fadeUp .4s ease"}}>
            ...
            {samplingPlan.map((sp,i)=>(
              <div key={i} style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:8}}>
                ...
                {sp.guide&&COLLECTION_GUIDES[sp.guide]&&(
                  <button onClick={()=>setShowGuide(sp.guide)} style={{marginTop:8,padding:"8px 14px",background:"color-mix(in srgb, var(--accent) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 13%, transparent)",borderRadius:8,color:"var(--accent)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}><I n="clip" s={14} c="var(--accent)" />Step-by-step collection guide</button>
                )}
              </div>
            ))}

            {/* COC Form */}
            <button onClick={initCOC} style={{width:"100%",padding:"14px 16px",marginTop:12,background:"color-mix(in srgb, var(--accent) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 15%, transparent)",borderRadius:12,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
              <I n="clip" s={18} c="var(--accent)" /><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:"var(--accent)"}}>Chain of Custody Form</div><div style={{fontSize:11,color:"var(--dim)"}}>Auto-filled from your plan · printable · free</div></div>
            </button>
            ...
            <div style={{marginTop:20}}>
              ...
              {(smart.building==="Pre-1986 home"||smart.concerns.includes("Lead worry")||smart.building==="School / Daycare")&&(
                <div style={{padding:"12px 16px",marginBottom:6,background:"color-mix(in srgb, var(--sev-high) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--sev-high) 13%, transparent)",borderRadius:12,fontSize:13,color:"var(--sev-high)",lineHeight:1.5}}>
              ...
              {(smart.trigger==="Someone got sick"||smart.concerns.includes("Illness"))&&(
                <div style={{padding:"12px 16px",marginBottom:6,background:"color-mix(in srgb, var(--danger) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--danger) 13%, transparent)",borderRadius:12,fontSize:13,color:"var(--danger)",lineHeight:1.5}}>
              ...
              {smart.concerns.includes("PFAS concern")&&(
                <div style={{padding:"12px 16px",marginBottom:6,background:"color-mix(in srgb, var(--warn) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--warn) 13%, transparent)",borderRadius:12,fontSize:13,color:"var(--warn)",lineHeight:1.5}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:16}}>
              <button onClick={startLab} style={{flex:1,padding:"14px 0",background:"linear-gradient(135deg,var(--accent2-deep),var(--accent2))",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Enter Lab Results →</button>
        ...
        {view==="fieldresults"&&(
          <div style={{paddingTop:28,paddingBottom:100,animation:"fadeUp .4s ease"}}>
            <div style={{fontSize:12,fontWeight:600,color:"var(--accent)",textTransform:"uppercase",letterSpacing:2,fontFamily:"var(--font-mono)",marginBottom:12}}>Sampling Recommendations</div>
            ...
            {samplingPlan.length===0?(
              ...
              <div key={i} style={{padding:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:14,marginBottom:10}}>
                ...
                <div style={{fontSize:12,color:"var(--sev-high)",fontWeight:600,marginBottom:6}}>Trigger: {sp.trigger}</div>
                ...
                <div style={{fontSize:11,color:"var(--accent)",fontFamily:"var(--font-mono)",marginTop:6}}>{sp.std}</div>
              </div>
            ))}

            {/* Sample Collection Guides */}
            {samplingPlan.length>0&&(
              <div style={{marginTop:12,padding:"14px 16px",background:"var(--card)",border:"1px solid color-mix(in srgb, var(--accent) 13%, transparent)",borderRadius:14}}>
                <div style={{fontSize:13,fontWeight:700,color:"var(--accent)",marginBottom:10}}>📋 Sample Collection Guides</div>
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            <button onClick={initCOC} style={{width:"100%",padding:"14px 16px",marginTop:16,background:"color-mix(in srgb, var(--accent) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 15%, transparent)",borderRadius:12,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
              <I n="clip" s={18} c="var(--accent)" /><div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:"var(--accent)"}}>Generate Chain of Custody Form</div><div style={{fontSize:11,color:"var(--dim)"}}>Auto-populates from sampling plan · printable</div></div>
            </button>
        ...
        {view==="labentry"&&(
          <div style={{paddingTop:28,paddingBottom:100,animation:"fadeUp .4s ease"}}>
            <div style={{fontSize:12,fontWeight:600,color:"var(--accent2)",textTransform:"uppercase",letterSpacing:2,fontFamily:"var(--font-mono)",marginBottom:12}}>Lab Results Entry</div>
            ...
            <div style={{marginBottom:16,padding:"14px 16px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:12}}>
              ...
              <select value={selState} onChange={e=>setSelState(e.target.value)} style={{width:"100%",padding:"10px 14px",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:8,color:"var(--c-text)",fontSize:14,fontFamily:"inherit"}}>
                <option value="">Federal (EPA) standards only</option>
                {Object.entries(STATE_STDS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
              </select>
              {selState&&<div style={{fontSize:11,color:"var(--warn)",fontFamily:"var(--font-mono)",marginTop:6}}>State standards will be applied in addition to federal MCLs where stricter</div>}
            </div>

            {/* PDF upload note */}
            <div style={{marginBottom:16,padding:"12px 16px",background:"color-mix(in srgb, var(--accent) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 13%, transparent)",borderRadius:10,fontSize:13,color:"var(--accent)",lineHeight:1.5}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                {Object.entries(QUICK_ADD).map(([k,v])=>(
                  <button key={k} onClick={()=>v.ids.forEach(id=>addLabParam(id))} style={{padding:"8px 14px",borderRadius:20,background:"color-mix(in srgb, var(--accent2) 6%, transparent)",border:"1px solid color-mix(in srgb, var(--accent2) 15%, transparent)",color:"var(--accent2)",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{v.label}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <select onChange={e=>{if(e.target.value)addLabParam(e.target.value);e.target.value="";}} style={{width:"100%",padding:"12px 14px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:10,color:"var(--c-text)",fontSize:14,fontFamily:"inherit"}}>
                    ...
                    <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,padding:"10px 12px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:10}}>
                      ...
                      {param.unit==="P/A"?(
                        <div style={{display:"flex",gap:4}}>
                          {["A","P"].map(v=><button key={v} onClick={()=>updateLabResult(r.id,"qualifier",v)} style={{padding:"6px 12px",borderRadius:6,background:r.qualifier===v?(v==="P"?"color-mix(in srgb, var(--danger) 13%, transparent)":"color-mix(in srgb, var(--success) 13%, transparent)"):"var(--surface)",border:`1px solid ${r.qualifier===v?(v==="P"?"var(--danger)":"var(--success)"):"var(--border)"}`,color:r.qualifier===v?(v==="P"?"var(--danger)":"var(--success)"):"var(--sub)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{v==="A"?"Absent":"Present"}</button>)}
                        </div>
                      ):(
                        <input type="number" value={r.value} onChange={e=>updateLabResult(r.id,"value",e.target.value)} placeholder="Result" style={{width:90,padding:"8px 10px",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:6,color:"var(--c-text)",fontSize:14,fontFamily:"var(--font-mono)",outline:"none",textAlign:"right"}} />
                      )}
            ...
            <button onClick={runEvaluation} disabled={labResults.length===0||labResults.every(r=>!r.value&&!r.qualifier)} style={{width:"100%",padding:"16px 0",background:labResults.length>0?"linear-gradient(135deg,var(--cta1),var(--cta2))":"var(--border)",border:"none",borderRadius:12,color:labResults.length>0?"#fff":"var(--dim)",fontSize:16,fontWeight:700,cursor:labResults.length>0?"pointer":"default",fontFamily:"inherit",boxShadow:labResults.length>0?"0 4px 20px color-mix(in srgb, var(--accent) 19%, transparent)":"none"}}>
        ...
        {view==="labresults"&&evaluation&&(
          <div style={{paddingTop:28,paddingBottom:100,animation:"fadeUp .4s ease"}}>
            ...
            <div style={{display:"flex",gap:4,padding:4,background:"var(--card)",borderRadius:10,border:"1px solid var(--border)",marginBottom:14,overflowX:"auto",scrollbarWidth:"none"}}>
              {[["compliance","shield","Compliance"],["chains","chain","Root Cause"],["actions","bolt","Actions"],["narrative","brain","Narrative"],["trending","chart","Trending"]].map(([k,ic,l])=><button key={k} onClick={()=>{setRTab(k);haptic("light");}} style={{flex:"0 0 auto",padding:"10px 16px",borderRadius:8,border:"none",background:rTab===k?"color-mix(in srgb, var(--accent) 8%, transparent)":"transparent",color:rTab===k?"var(--accent)":"var(--dim)",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:6}}><I n={ic} s={16} c={rTab===k?"var(--accent)":"var(--dim)"} />{l}</button>)}
            </div>
            ...
            {rTab==="compliance"&&(
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {evaluation.findings.map((f,i)=>{
                  ...
                  const color = hasV ? "var(--danger)" : hasA ? "var(--warn)" : "var(--success)";
                  return (
                    <div key={i} style={{padding:"13px 15px",background:"var(--card)",border:`1px solid ${mix(color,15)}`,borderRadius:R.md}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                        ...
                        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
                          ...
                          <span style={{display:"inline-flex",alignItems:"center",gap:5,fontSize:10,fontWeight:700,letterSpacing:.5,textTransform:"uppercase",color,background:mix(color,10),border:`1px solid ${mix(color,33)}`,borderRadius:999,padding:"3px 9px",whiteSpace:"nowrap"}}><span style={{width:5,height:5,borderRadius:999,background:color}} />{hasV?"Violation":hasA?"Advisory":"Within limits"}</span>
                        </div>
                      </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {chains.length===0?(
                  ...
                  <div key={i} style={{padding:16,background:"var(--card)",border:`1px solid ${sevColor(ch.severity)}25`,borderRadius:14}}>
                    ...
                    <div style={{marginTop:8,padding:"10px 14px",background:"var(--surface)",borderRadius:8,fontSize:13,color:"var(--sub)",lineHeight:1.6}}><strong style={{color:"var(--accent)"}}>Recommendation:</strong> {ch.recommendation}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {rTab==="actions"&&recs&&(
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {[{k:"immediate",l:"Immediate Actions",c:"var(--danger)",i:"alert"},{k:"shortTerm",l:"Short-Term (30 Days)",c:"var(--sev-high)",i:"bolt"},{k:"longTerm",l:"Long-Term",c:"var(--accent)",i:"pipe"},{k:"monitoring",l:"Ongoing Monitoring",c:"var(--sub)",i:"refresh"}].map(cat=>{
                  ...
                  return (
                    <div key={cat.k} style={{padding:14,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12}}>
                      ...
                      {recs[cat.k].map((r,j)=><div key={j} style={{fontSize:14,color:"var(--text)",lineHeight:1.6,marginBottom:6,paddingLeft:12,borderLeft:`2px solid ${mix(cat.c,19)}`}}>{r}</div>)}
                    </div>
                  );
                })}
                ...
                {(evaluation.tier==="immediate"||evaluation.tier==="advisory"||chains.length>0)&&(
                  <div style={{padding:16,background:"color-mix(in srgb, var(--accent2) 6%, transparent)",border:"1px solid color-mix(in srgb, var(--accent2) 15%, transparent)",borderRadius:14,marginTop:6}}>
                    <div style={{fontSize:14,fontWeight:700,color:"var(--accent2)",marginBottom:6}}>Professional Review Recommended</div>
                        ...
                        return (
                          <div key={pid} style={{padding:"14px 16px",background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,marginBottom:8}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                              ...
                              <span style={{fontSize:12,fontFamily:"var(--font-mono)",color:trend>0?"var(--danger)":trend<0?"var(--success)":"var(--sub)"}}>{trend>0?"↑":"↓"} {Math.abs(trend).toFixed(2)} {param.unit}</span>
                            </div>
                            ...
                            <div style={{display:"flex",gap:3,alignItems:"flex-end",height:48,marginBottom:6}}>
                              {points.map((p,i) => {
                                ...
                                return <div key={i} style={{flex:1,height:Math.max(h,2),background:overLimit?"var(--danger)":"var(--accent)",borderRadius:"3px 3px 0 0",opacity:0.6 + (i/points.length)*0.4}} title={`${new Date(p.ts).toLocaleDateString()}: ${p.value} ${param.unit}`} />;
                              })}
                            </div>
                            {limit && <div style={{height:1,background:"color-mix(in srgb, var(--danger) 25%, transparent)",marginBottom:4}} />}
                            <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--dim)",fontFamily:"var(--font-mono)"}}>
                              ...
                              {limit&&<span style={{color:"color-mix(in srgb, var(--danger) 50%, transparent)"}}>Limit: {limit} {param.unit}</span>}
        ...
        {view==="coc"&&coc&&(
          <div style={{paddingTop:28,paddingBottom:100,animation:"fadeUp .4s ease"}} id="coc-form">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div>
                <div style={{fontSize:12,fontWeight:600,color:"var(--accent)",textTransform:"uppercase",letterSpacing:2,fontFamily:"var(--font-mono)"}}>Chain of Custody</div>
                <div style={{fontSize:11,color:"var(--c-dim)",fontFamily:"var(--font-mono)",marginTop:2}}>Drinking Water Samples</div>
              </div>
              <button onClick={()=>window.print()} style={{padding:"8px 16px",background:"color-mix(in srgb, var(--accent) 8%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 19%, transparent)",borderRadius:8,color:"var(--accent)",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}><I n="download" s={14} c="var(--accent)" />Print</button>
            </div>

            {/* Header */}
            <div style={{padding:"16px 20px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:"14px 14px 0 0",display:"flex",alignItems:"center",gap:12}}>
              <Logo s={32} />
              <div><div style={{fontSize:16,fontWeight:800}}>Hydro<span style={{color:"var(--accent)"}}>Scan</span></div><div style={{fontSize:10,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Chain of Custody · Prudence EHS</div></div>
              <div style={{marginLeft:"auto",textAlign:"right",fontSize:10,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>{clock.toLocaleDateString()}<br/>{clock.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>
            </div>

            {/* Project Info */}
            <div style={{padding:"14px 18px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderTop:"none"}}>
              <div style={{fontSize:10,fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Project Information</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                {[{k:"project",l:"Project"},{k:"client",l:"Client"},{k:"siteAddr",l:"Site Address",span:true},{k:"sampler",l:"Sampler"},{k:"samplerCo",l:"Company"},{k:"samplerPhone",l:"Phone"},{k:"samplerEmail",l:"Email"},{k:"labName",l:"Laboratory"},{k:"labAcct",l:"Lab Acct #"},{k:"labAccred",l:"Lab Accreditation (NELAP/TNI)"},{k:"labISO",l:"ISO/IEC 17025?"},{k:"turnaround",l:"TAT"},{k:"dataPackage",l:"Data Package Level"}].map(f=>(
                  <div key={f.k} style={f.span?{gridColumn:"1/-1"}:{}}>
                    <div style={{fontSize:9,color:"var(--c-dim)",fontFamily:"var(--font-mono)",marginBottom:2}}>{f.l}</div>
                    {f.k==="turnaround"?<select value={coc[f.k]} onChange={e=>setCoc(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option>Rush (24h)</option><option>Expedited (3 days)</option><option>Standard (10 days)</option></select>
                    :f.k==="dataPackage"?<select value={coc[f.k]} onChange={e=>setCoc(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option>Summary report only</option><option>Level II — QC summary</option><option>Level III — full QC + raw data</option><option>Level IV — complete + calibrations</option></select>
                    :f.k==="labISO"?<select value={coc[f.k]||""} onChange={e=>setCoc(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option value="">Not verified</option><option>Yes — ISO/IEC 17025</option><option>No</option></select>
                    :<input value={coc[f.k]||""} onChange={e=>setCoc(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} onFocus={e=>e.target.style.borderColor="var(--accent)"} onBlur={e=>e.target.style.borderColor="var(--c-border)"} />}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{padding:"14px 18px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderTop:"none"}}>
              <div style={{fontSize:10,fontWeight:700,color:"var(--warn)",textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Quality Control Samples</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>
                {[{k:"fieldBlank",l:"Field Blank",tip:"DI water poured into sample container on-site — proves no ambient contamination"},{k:"tripBlank",l:"Trip Blank",tip:"Lab-sealed bottle transported with samples — proves no transport contamination (required for VOCs)"},{k:"duplicate",l:"Duplicate Sample",tip:"Second sample from same tap — proves lab precision/repeatability"},{k:"equipBlank",l:"Equipment Blank",tip:"DI water through sampling equipment — proves no equipment contamination"}].map(qc=>(
                  <button key={qc.k} onClick={()=>setCoc(p=>({...p,qcSamples:{...p.qcSamples,[qc.k]:!p.qcSamples[qc.k]}}))} style={{padding:"8px 14px",borderRadius:8,background:coc.qcSamples?.[qc.k]?"color-mix(in srgb, var(--warn) 8%, transparent)":"var(--c-surface)",border:`1px solid ${coc.qcSamples?.[qc.k]?"color-mix(in srgb, var(--warn) 25%, transparent)":"var(--c-border)"}`,color:coc.qcSamples?.[qc.k]?"var(--warn)":"var(--c-sub)",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>{coc.qcSamples?.[qc.k]?"✓ ":""}{qc.l}</button>
                ))}
              </div>
              {!coc.qcSamples?.fieldBlank&&!coc.qcSamples?.tripBlank&&<div style={{fontSize:11,color:"var(--danger)",lineHeight:1.5,padding:"6px 10px",background:"color-mix(in srgb, var(--danger) 3%, transparent)",borderRadius:6}}>⚠ No QC samples selected. Without blanks, sample integrity cannot be verified. Results may be challenged in legal proceedings.</div>}
              <div style={{display:"flex",gap:8,marginTop:8}}>
                <div style={{flex:1}}><div style={{fontSize:9,color:"var(--c-dim)",fontFamily:"var(--font-mono)",marginBottom:2}}>Tamper-Evident Seals</div><select value={coc.tamperSeals||""} onChange={e=>setCoc(p=>({...p,tamperSeals:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option>Yes — applied to all bottles</option><option>Yes — applied to cooler only</option><option>No seals used</option></select></div>
                <div style={{flex:1}}><div style={{fontSize:9,color:"var(--c-dim)",fontFamily:"var(--font-mono)",marginBottom:2}}>Seal Condition on Receipt</div><select value={coc.sealCondition||""} onChange={e=>setCoc(p=>({...p,sealCondition:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option value="">Lab use</option><option>Intact</option><option>Broken / compromised</option></select></div>
              </div>
            </div>
            ...
            <div style={{padding:"14px 18px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderTop:"none"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{fontSize:10,fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:1.5}}>Samples ({coc.samples.length})</div>
                <button onClick={addCocSample} style={{padding:"3px 10px",borderRadius:5,background:"color-mix(in srgb, var(--accent) 8%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 19%, transparent)",color:"var(--accent)",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>+ Add</button>
              </div>
              {coc.samples.map((s,i)=>(
                <div key={i} style={{padding:"10px 12px",background:"var(--c-surface)",borderRadius:8,marginBottom:5,position:"relative"}}>
                  <button onClick={()=>removeCocSample(i)} style={{position:"absolute",top:6,right:6,background:"none",border:"none",color:"var(--c-faint)",fontSize:13,cursor:"pointer"}}>×</button>
                  <div style={{display:"grid",gridTemplateColumns:"70px 1fr 1fr",gap:5,marginBottom:5}}>
                    <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>ID</div><input value={s.id} onChange={e=>updateCocSample(i,"id",e.target.value)} style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--accent)",fontSize:11,fontFamily:"var(--font-mono)",fontWeight:700,outline:"none",boxSizing:"border-box"}} /></div>
                    <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Collected</div><input type="datetime-local" value={s.datetime} onChange={e=>updateCocSample(i,"datetime",e.target.value)} style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                    <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Matrix</div><select value={s.matrix} onChange={e=>updateCocSample(i,"matrix",e.target.value)} style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit"}}><option>Drinking Water</option><option>Groundwater</option><option>Surface Water</option></select></div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:5}}>
                    <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Location</div><input value={s.location} onChange={e=>updateCocSample(i,"location",e.target.value)} placeholder="Kitchen cold, first draw" style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                    <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Container / Preservative</div><input value={s.preservative} onChange={e=>updateCocSample(i,"preservative",e.target.value)} placeholder="250mL HDPE / HNO₃" style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                  </div>
                  <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Analyses Requested</div><input value={s.analyses} onChange={e=>updateCocSample(i,"analyses",e.target.value)} placeholder="Lead, Copper, pH — EPA 200.8" style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                </div>
              ))}
            </div>
            ...
            <div style={{padding:"14px 18px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderTop:"none"}}>
              <div style={{fontSize:10,fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:1.5,marginBottom:6}}>Instructions & Receipt</div>
              <textarea value={coc.specialInstructions} onChange={e=>setCoc(p=>({...p,specialInstructions:e.target.value}))} placeholder="Stagnation: 8 hrs. First-draw per EPA 3Ts. Include field blank." rows={2} style={{width:"100%",padding:"8px 10px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit",outline:"none",resize:"vertical",boxSizing:"border-box",marginBottom:6}} />
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                <div><div style={{fontSize:9,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Temp on Receipt (°C)</div><input value={coc.tempOnReceipt||""} onChange={e=>setCoc(p=>({...p,tempOnReceipt:e.target.value}))} placeholder="Lab use" style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                <div><div style={{fontSize:9,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Cooler Intact?</div><select value={coc.coolerIntegrity||""} onChange={e=>setCoc(p=>({...p,coolerIntegrity:e.target.value}))} style={{width:"100%",padding:"7px 8px",background:"var(--c-surface)",border:"1px solid var(--c-border)",borderRadius:5,color:"var(--c-text)",fontSize:12,fontFamily:"inherit"}}><option value="">Lab use</option><option>Intact — ice present</option><option>Intact — no ice</option><option>Compromised</option></select></div>
              </div>
            </div>

            {/* Custody Transfer */}
            <div style={{padding:"14px 18px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderTop:"none",borderRadius:"0 0 14px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{fontSize:10,fontWeight:700,color:"var(--accent)",textTransform:"uppercase",letterSpacing:1.5}}>Custody Transfer</div>
                <button onClick={addCustodyRow} style={{padding:"3px 10px",borderRadius:5,background:"color-mix(in srgb, var(--accent) 8%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 19%, transparent)",color:"var(--accent)",fontSize:10,cursor:"pointer",fontFamily:"inherit"}}>+ Row</button>
              </div>
              {coc.custody.map((c,i)=>(
                <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,padding:"8px 10px",background:"var(--c-surface)",borderRadius:6,marginBottom:4}}>
                  <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Relinquished By</div><input value={c.from} onChange={e=>updateCustody(i,"from",e.target.value)} placeholder="Name / Signature" style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                  <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Date/Time</div><input type="datetime-local" value={c.fromDate} onChange={e=>updateCustody(i,"fromDate",e.target.value)} style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                  <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Received By</div><input value={c.to} onChange={e=>updateCustody(i,"to",e.target.value)} placeholder="Name / Signature" style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                  <div><div style={{fontSize:8,color:"var(--c-dim)",fontFamily:"var(--font-mono)"}}>Date/Time</div><input type="datetime-local" value={c.toDate} onChange={e=>updateCustody(i,"toDate",e.target.value)} style={{width:"100%",padding:"5px 6px",background:"var(--c-card)",border:"1px solid var(--c-border)",borderRadius:4,color:"var(--c-text)",fontSize:11,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}} /></div>
                </div>
              ))}
              <div style={{fontSize:9,color:"var(--c-faint)",marginTop:6}}>Signatures confirm unbroken custody from collection to laboratory receipt.</div>
            </div>

            <div style={{marginTop:12,padding:"10px 14px",background:"color-mix(in srgb, var(--accent) 3%, transparent)",border:"1px solid color-mix(in srgb, var(--accent) 7%, transparent)",borderRadius:10,fontSize:10,color:"var(--c-dim)",lineHeight:1.6}}>Generated by HydroScan · Prudence Safety & Environmental Consulting, LLC · This form does not replace laboratory-specific COC requirements.</div>

            <div style={{display:"flex",gap:8,marginTop:14}}>
              <button onClick={()=>window.print()} style={{flex:1,padding:"14px 0",background:"linear-gradient(135deg,var(--cta1),var(--cta2))",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Print / Save as PDF</button>
              <button onClick={()=>setView("dash")} style={{padding:"14px 20px",background:"transparent",border:"1px solid var(--c-border)",borderRadius:10,color:"var(--c-sub)",fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>Done</button>
            </div>
          </div>
        )}
      </div>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;}button{font-family:inherit;}
        ...
        .ai-brain{filter:drop-shadow(0 0 5px rgba(119,178,88,.6));}
        ...
        select option{background:var(--c-card);color:var(--c-sub);}
        ::-webkit-scrollbar{width:4px;height:0;}::-webkit-scrollbar-track{background:transparent;}::-webkit-scrollbar-thumb{background:var(--c-border);border-radius:2px;}
      `}</style>
      ...
      {!showTour&&(()=>{
        ...
        return (
          <nav style={{position:"fixed",left:0,right:0,bottom:0,zIndex:120,display:"flex",justifyContent:"center",background:"color-mix(in srgb, var(--surface) 92%, transparent)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderTop:`1px solid var(--border)`,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
            ...
            <svg aria-hidden="true" width="0" height="0" style={{position:"absolute",width:0,height:0}}><defs><linearGradient id="aiBrainGrad" gradientUnits="userSpaceOnUse" x1="2" y1="3" x2="22" y2="21"><stop offset="0%" stopColor="#EAFBE6"/><stop offset="48%" stopColor="#A7E08B"/><stop offset="100%" stopColor="#3F7A2E"/></linearGradient></defs></svg>
                  ...
                  return (
                    <button key={t.k} onClick={()=>{haptic("light");t.onTap();}} aria-label="HydroScan AI" className="tap navtab" style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"9px 4px 10px",background:"transparent",border:"none",cursor:"pointer",fontFamily:"inherit"}}>
                      <div className="ai-brain" style={{position:"relative",width:30,height:28,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <span className="ai-halo" style={{position:"absolute",inset:-9,borderRadius:"50%",background:"radial-gradient(circle, rgba(119,178,88,0.5) 0%, rgba(119,178,88,0) 70%)",pointerEvents:"none"}} />
