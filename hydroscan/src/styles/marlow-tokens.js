 ...
 * All rights reserved.
 *
 * AtmosFlow AI (Marlow) design tokens & primitives.
 *
 * Extracted from FieldAssistant.jsx during the Phase 4 design-system
 * pass so other AI surfaces (MarlowWatchPanel, future inline-AI
 ...
 * (`MARLOW_*`) so a consumer importing both won't see collisions.
 *
...
export const MARLOW_STAGGER_MS = 60
...
export const marlowAtmosphere = (base = 'var(--card)', strengthPct = 7) =>
...
export const MARLOW_SHEET_SHADOW =
...
export const marlowComposerFocusShadow = () =>
...
export const MARLOW_CHIP_TONES = {
  accent: {
    fg: 'var(--accent)',
 ...
 * the Marlow primitives depend on. A consumer surface mounts this
 * once (typically as `<style>{MARLOW_KEYFRAMES_CSS}</style>` at the
 ...
 */
export const MARLOW_KEYFRAMES_CSS = `
@keyframes marlowReveal {
  0%   { opacity: 0; transform: translateY(6px); }
  100% { opacity: 1; transform: translateY(0); }
}
@keyframes marlowBackdropIn {
  from { opacity: 0; }
  to   { opacity: 1; }
}
@keyframes marlowSheetIn {
  from { opacity: 0; transform: translateY(28px); }
  to   { opacity: 1; transform: translateY(0); }
}
@keyframes marlowMsgIn {
  0%   { opacity: 0; transform: translateY(8px) scale(0.98); }
  100% { opacity: 1; transform: translateY(0) scale(1); }
}
@keyframes marlowChipIn {
  0%   { opacity: 0; transform: translateY(-4px); }
  100% { opacity: 1; transform: translateY(0); }
}
.marlow-msg-in {
  animation: marlowMsgIn ${MARLOW_DURATION.enter}ms ${MARLOW_EASE_OUT} both;
}
.marlow-chip-in {
  display: inline-flex;
  animation: marlowChipIn ${MARLOW_DURATION.enter}ms ${MARLOW_EASE_OUT} both;
}
.marlow-suggestion:hover:not(:disabled),
.marlow-suggestion:focus-visible {
...
.marlow-suggestion:hover:not(:disabled) .marlow-suggestion__icon,
.marlow-suggestion:focus-visible .marlow-suggestion__icon {
  background: color-mix(in srgb, var(--accent) 18%, transparent) !important;
}
.marlow-suggestion:active:not(:disabled) {
