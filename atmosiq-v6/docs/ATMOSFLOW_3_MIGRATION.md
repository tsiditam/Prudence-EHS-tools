...
| **4** | Data model: migration 029 adds nullable `engine_version` (immutable-once-set trigger) + `standards_manifest` + `investigation` jsonb columns; legacy score columns already nullable and untouched; typed persistence contract (`persistence.ts`) with a lossless round-trip guard | 28 | ✅ **Landed** |
| 5 | Consumer data surface: expose the framework result to Jasper/report/analytics via the connectivity layer. NOTE: assessments persist client-side via Supabase RLS (no REST assessments API to version); the consumer-facing read shape is coupled to the renderer, so it lands with Stages 6–7 | 29 | Planned (with 6/7) |
