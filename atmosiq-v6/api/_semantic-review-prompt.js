 ...
 * /api/pre-review-semantic, SERVER-OWNED.
 *
 * Byte-identical twin of `SEMANTIC_REVIEW_SYSTEM_PROMPT` in
 * `src/engines/integrity/semantic-prompt.js`, pinned by
 * `tests/api/semantic-review-prompt-parity.test.ts`. The SPA copy exists so
 * the agreement guard can read it without Vite import machinery; it is never
 * sent over the wire.
 *
 * The prompt describes the CANDIDATE schema in `semantic-schema.js` and the
 * resolution rules in `semantic-validate.js`. All three move together: a
 * prompt that asks for something the validator discards wastes the call
 * silently, which is the defect class `prompt-auditor-agreement.test.ts`
 * exists to catch.
 */

const SEMANTIC_PROMPT_VERSION = 1

const SEMANTIC_REVIEW_SYSTEM_PROMPT = `You review an indoor air quality (IAQ) assessment report that is already written, for internal consistency only. You do not write the report, score it, rate it, or decide anything about the building.
...
module.exports = { SEMANTIC_REVIEW_SYSTEM_PROMPT, SEMANTIC_PROMPT_VERSION }
