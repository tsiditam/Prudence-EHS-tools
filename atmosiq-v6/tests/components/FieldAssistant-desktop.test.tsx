...
/**
 * FieldAssistant — desktop panel mode.
 *
 * On desktop (>= 1024px) the assistant is a panel docked to the right edge
 * of the window beside the work, not a phone sheet floating over a scrim:
 * no backdrop, pinned right at the panel width, and a greeting on the empty
 * canvas instead of the watermark. The phone chrome is pinned by
...
describe('FieldAssistant desktop panel mode', () => {
  it('drops the scrim and docks to the right edge at the panel width', () => {
    const { container } = render(<FieldAssistant onClose={() => {}} context={{}} desktop panelWidth={420} />)
    ...
    expect(sheet.style.left).toBe('auto')
  ...
  it('greets on the empty canvas instead of showing the watermark', () => {
    render(<FieldAssistant onClose={() => {}} context={{}} desktop />)
    expect(screen.getByTestId('jasper-desktop-greeting')).toBeTruthy()
    expect(screen.getByText('Ask about this investigation')).toBeTruthy()
  })

  it('does not replay a New chat bump on mount (a fresh mount is already a fresh chat)', () => {
    // Mounting with a nonce that was bumped while the panel was closed must
    // not touch the surface — it is empty already and stays that way.
    const { container } = render(<FieldAssistant onClose={() => {}} context={{}} desktop newChatNonce={3} />)
    expect(screen.getByTestId('jasper-desktop-greeting')).toBeTruthy()
