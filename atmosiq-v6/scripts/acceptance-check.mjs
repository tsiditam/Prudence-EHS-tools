...
const grepMatches = (pattern, paths, { mustMatch = true, minCount = 1 } = {}) => { const args = ["-rnE", pattern, ...paths]; const proc = spawnSync("grep", args, { cwd: REPO_ROOT, encoding: "utf8" }); if (proc.status === 2) return { ok: false, detail: `grep error: ${proc.stderr.trim()}` }; const matches = proc.stdout.trim().split("\n").filter(Boolean); const count = proc.status === 0 ? matches.length : 0; if (mustMatch) { if (count < minCount) return { ok: false, detail: `pattern ${pattern} matched ${count} times; need >= ${minCount}` }; return { ok: true, detail: `${count} matches` }; } else { if (count > 0) return { ok: false, detail: `pattern ${pattern} matched ${count} times; expected 0\n${matches.slice(0, 5).join("\n")}` }; return { ok: true, detail: `0 matches (as required)` }; } };
