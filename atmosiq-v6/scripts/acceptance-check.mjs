...
// A path list is an either/or: several criteria name `api/webhook.ts` and
// `api/webhook.js` and expect whichever exists. But an EXCLUSION check whose
// paths have all vanished asserts nothing, and used to return ok — so deleting
// the file a removal-guard watched made that guard silently pass. An exclusion
// with nothing left to read is a broken criterion, not a satisfied one.
const grepMatches = (pattern, paths, { mustMatch = true, minCount = 1 } = {}) => { const existing = paths.filter((p) => existsSync(resolvePath(p))); if (existing.length === 0) return { ok: false, detail: `none of the paths exist: ${paths.join(", ")}${mustMatch ? "" : " — an exclusion check with no readable path proves nothing; re-point it"}` }; const args = ["-rnE", pattern, ...existing]; const proc = spawnSync("grep", args, { cwd: REPO_ROOT, encoding: "utf8" }); if (proc.status === 2) return { ok: false, detail: `grep error: ${proc.stderr.trim()}` }; const matches = proc.stdout.trim().split("\n").filter(Boolean); const count = proc.status === 0 ? matches.length : 0; if (mustMatch) { if (count < minCount) return { ok: false, detail: `pattern ${pattern} matched ${count} times; need >= ${minCount}` }; return { ok: true, detail: `${count} matches across ${existing.length} path(s)` }; } else { if (count > 0) return { ok: false, detail: `pattern ${pattern} matched ${count} times; expected 0\n${matches.slice(0, 5).join("\n")}` }; return { ok: true, detail: `0 matches (as required)` }; } };
