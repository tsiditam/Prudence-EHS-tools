...
export const STANDARDS_CORPUS = [
  ...
  },
  {
    ...
    citation: 'ASHRAE 62.1-2025 §6.4 (DCV); Δ700 ppm — informative appendix of earlier ASHRAE 62.1 editions, since removed; ASHRAE Position Document on Indoor Carbon Dioxide (2022)',
    document: 'ASHRAE-62.1',
    year: 2025,
    tags: ['ashrae', '62.1', 'co2', 'ventilation', 'dcv', 'demand-controlled'],
    text: 'CO₂ is widely used as a proxy for ventilation adequacy in human-occupied spaces because exhaled CO₂ from occupants is the primary indoor source. ASHRAE 62.1-2025 does not specify an absolute indoor CO₂ limit and its current text carries no CO₂ differential either; the indoor-outdoor differential is a diagnostic that came from an informative appendix of earlier 62.1 editions, since removed (see the ASHRAE Position Document on Indoor Carbon Dioxide, 2022). At steady-state occupancy, a sustained differential of approximately 700 ppm above outdoor (typically ~420 ppm) corresponds to ventilation roughly matching the VRP Rp component for office activity, yielding indoor concentrations near 1100–1200 ppm. Higher differentials suggest under-ventilation; concentrations above 1500 ppm correlate in some studies with elevated reports of headache, drowsiness, and decreased cognitive performance, though CO₂ itself is not toxic at these levels. Demand-controlled ventilation (DCV) modulates outdoor air to maintain a CO₂ setpoint, reducing energy use during low occupancy. DCV is permitted by ASHRAE 62.1 when zones have variable occupancy and outdoor air rates are not driven by the area component alone. Field measurement requires NDIR sensors with a current calibration (AtmosFlow treats a calibration as current for 365 days); always pair indoor readings with a simultaneous outdoor reading.',
  },
  {
    id: 'co2-ventilation-estimation-methods',
    title: 'Estimating outdoor-air delivery from indoor CO₂',
    citation: 'ASTM D6245-18, Standard Guide for Using Indoor Carbon Dioxide Concentrations to Evaluate Indoor Air Quality and Ventilation; Persily & de Jonge (2017) Indoor Air 27(5): 868–879; Batterman (2017) IJERPH 14(2): 145',
    document: 'IAQ-Methodology',
    year: 2018,
    tags: ['co2', 'ventilation', 'mass-balance', 'decay', 'steady-state', 'ach', 'cfm', 'astm-d6245'],
    text: 'Indoor CO₂ can be used to ESTIMATE outdoor-air delivery in occupied spaces, because occupants are the dominant indoor source and the generation rate per person is known within about ±10% for a given activity level (Persily & de Jonge 2017). Two methods are in common use. The steady-state method, Vo = G × 10⁶ / (Cin − Cout), gives outdoor air per person from one indoor/outdoor pair, but is valid only once the space has reached equilibrium — roughly three air-change time constants of steady occupancy — and its largest error is the assumed generation rate G. The decay (transient) method fits ln(C − Cout) against time after occupants leave, ACH = ln((C₀ − Cout)/(Cₜ − Cout)) / Δt; it needs no generation rate and is the stronger estimate when an unoccupied period exists, yielding air changes per hour directly (Batterman 2017). Both are estimates of ventilation, not measurements of it: outdoor CO₂ must be measured rather than assumed, differentials below about 50 ppm are unreliable, and for compliance documentation a direct airflow measurement (duct traverse, balometer, or tracer gas per ASTM E741) outranks any CO₂-derived figure. Neither method makes CO₂ a contaminant limit (see the ASHRAE Position Document on Indoor Carbon Dioxide, 2022).',
  },
  {
    id: 'ashrae-241-2023',
