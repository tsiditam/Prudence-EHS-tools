...
export function buildEvidenceTraceabilityMatrix(graphContext) {
  ...
  const out = [
    p(
      'Each flagged finding is traced to the field evidence that supports or conflicts with it and to the standards it references. Relationships are derived from the deterministic scoring graph — the same basis used elsewhere in this report — and support, but do not confirm, interpretation. Standards shown as "reference" are used to gauge adequacy and are not health or compliance limits.',
