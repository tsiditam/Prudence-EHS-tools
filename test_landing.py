...
check("Findings section present (between workflow and the draft-report payoff)", ('id="findings"' in html) and ("Turn Assessment Data Into Actionable Findings" in html) and ("Intelligent Findings &amp; Prioritization" in html) and (html.index('How AtmosFlow Works') < html.index('id="findings"') < html.index('Guided Assessment. Draft Reports in Minutes.')))
check("Findings section has the four value bullets", all(t in html for t in ["Prioritize What Matters","Confidence You Can Trust","Actionable Recommendations","Built for Professional Review"]))
check("Findings trust statement present", "Screening support only. Professional review remains in control." in html)
check("Findings screenshot embedded", 'src="/ss-findings-priority.jpeg"' in html)
check("Findings score ring animates once on view (0 to 19)", ('class="score-ring"' in html) and ('id="findScore"' in html) and ('.score-ring.run .ring-prog' in html) and ("getElementById('findScore')" in html))
